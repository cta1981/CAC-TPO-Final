...
class Inventario:
    ...
    def consultar_producto(self, codigo):
        ...
        row = self.cursor.fetchone() #Recupera la primera fila de resultados de la consulta realizada
        if row:
    ...
    def modificar_producto(self, codigo, nueva_descripcion, nueva_cantidad, nuevo_precio):
        ...
        if producto:
            ...
            self.cursor.execute(sql)
            self.conexion.commit() #e realiza una confirmación explícita de la transacción en la base de datos para guardar los cambios de la actualización. 

    def eliminar_producto(self, codigo):
        sql = f'DELETE FROM productos WHERE codigo = {codigo};' 
        self.cursor.execute(sql)
        if self.cursor.rowcount > 0: #número de filas afectadas por la operación de eliminación
...
mi_inventario.agregar_producto(3, 'Monitor 22 pulg', 15, 52500)
mi_inventario.agregar_producto(4, 'Monitor 40 pulg', 4, 80000)
mi_inventario.agregar_producto(5, 'Impresora', 2, 150000)

# Listar todos los productos del inventario
mi_inventario.listar_productos()

# modificar del inventario
mi_inventario.modificar_producto(3, "Micrófono", 5, 5000)
...
mi_carrito.agregar(2, 1, mi_inventario) # Agregar 4 unidad del producto con código 3 al carrito

# Listar todos los productos del carrito
mi_carrito.mostrar()

# Quitar 1 producto al carrito
mi_carrito.quitar (3, 1, mi_inventario) # Quitar 1 unidad del producto con código 1 al carrito
